...
function MyApp(props){
  ...
  addItem = (item) => {
    if(state.cart==null||state.cart==undefined){
      //if state.cart  is refreshed after user login
    setState({cart:cart});
    state.cart=cart;
  }
  ...
  return (
    <AppContext.Provider value={{cart: state.cart, addItem: addItem, removeItem: removeItem,isAuthenticated:!!(state.user),user:state.user,setUser:setUser}}>
      <Head>
        <link
          rel="stylesheet"
